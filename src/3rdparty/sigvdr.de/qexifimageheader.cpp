...
QExifImageHeader::Thumbnail QExifImageHeader::thumbnailData() const
{
    // returning QByteArray takes constant time, because QByteArray is implicitly shared
    return { d->thumbnailData, d->thumbnailOrientation.isNull() ? (quint16)0 : d->thumbnailOrientation.toShort() };
}
...
QImage QExifImageHeader::thumbnail() const
{
    ...
    if (!d->thumbnailOrientation.isNull()) {
        qDebug() << Q_FUNC_INFO << d->thumbnailOrientation.toShort();
...
QList<ExifIfdHeader> QExifImageHeader::readIfdHeaders(QDataStream &stream) const
{
    ...
    for (quint16 i = 0; i < count; i++) {
        ...
        stream >> header;
        qDebug("header %d 0x%04X of type %d", i+1, header.tag, header.type);
...
QExifValue QExifImageHeader::readIfdValue(QDataStream &stream, int startPos, const ExifIfdHeader &header) const
{
    ...
    case QExifValue::Ascii:
        if (header.count > 4) {
            ...
            qDebug("header 0x%04X = '%s'", header.tag, ascii.constData());
            return QExifValue(QString::fromLatin1(ascii.constData(), ascii.size() - 1));
        } else {
            qDebug("header 0x%04X = '%s'", header.tag, header.offsetAscii);
    ...
    case QExifValue::Short:
    {
        ...
        qDebug("header 0x%04X = %d", header.tag, value.size() ? value.front() : 0);
        return QExifValue(value);
    }
    case QExifValue::Long:
    {
...
bool QExifImageHeader::read(QIODevice *device)
{
    ...
    QList<ExifIfdHeader> headers = readIfdHeaders(stream);
    qDebug("OFFSET 0x%04X", device->pos());
    stream >> offset;
