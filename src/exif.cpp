...
bool Exif::File::load(const QString& fileName, bool createIfEmpty)
{
    ...
    mExifData = exif_data_new_from_file(mFileName.toLocal8Bit().constData());
    if (!mExifData)
    {
        if (!createIfEmpty)
            return false;
...
{
    JPEGData * jpegData = jpeg_data_new_from_file(mFileName.toLocal8Bit().constData());
    jpeg_data_set_exif_data(jpegData, mExifData);
    return jpeg_data_save_file(jpegData, fileName.toLocal8Bit().constData());
}
